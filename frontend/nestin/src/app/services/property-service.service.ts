...
import { Observable, forkJoin, map, switchMap, catchError, of, throwError, expand, reduce, EMPTY } from 'rxjs';
...
export class PropertyService {
  ...
  private apiUrl1 = 'https://localhost:7026/api/v1/Properties/{id}';
  private apiUrl2 = 'https://localhost:7026/api/v1/Amenities';
  private apiUrl3 = 'https://localhost:7026/api/v1/AmenityCategories';
  private apiUrl4 = 'https://localhost:7026/api/v1/PropertyTypes';
  private apiUrl5 = 'https://localhost:7026/api/v1/Bookings';
...
private getImagesForProperty(propertyId: string): Observable<IPropertyImage[]> {
  return this.http.get<IPropertyInfo>(
    ...
    map(propertyInfo => {
      ...
      return propertyInfo.photos.map(photo => ({
        PropertyID: propertyId,  
        PhotoId: photo.id,
        TouchedAt: new Date().toISOString()
      } as IPropertyImage)).sort((a, b) => 
 ...
 getPropertyById(id: string): Observable<IPropertyInfo> {
    return this.http.get<IPropertyInfo>(`${this.apiUrl}/${id}`).pipe(
  ...
  getAmenitiesByPropertyId(propertyId: string): Observable<IPropertyAmenityItem> {
    return this.http.get<IPropertyAmenityItem>(`${this.apiUrl}/${propertyId}/Amenities`);
  }
  ...
  private getAmenitiesPage(page: number, pageSize: number): Observable<{ items: IPropertyAmenity[], metaData: any }> {
    ...
    return this.http.get<{ items: IPropertyAmenity[], metaData: any }>(this.apiUrl2, { params });
  }
  ...
  getPropertyTypes(): Observable<{ items: IPropertyType[], metaData: any }> {
    return this.http.get<{ items: IPropertyType[], metaData: any }>(ApiConstant.PropertiesApi.getAllPropertyTypes);
  }
  ...
  private getLocationPage(page: number, pageSize: number): Observable<{ items: ILocation[]; metaData: any }> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('pageSize', pageSize.toString());

    return this.http.get<{ items: ILocation[]; metaData: any }>(ApiConstant.location.getAllLocations, { params });
  }
  ...
  getAllLocations(): Observable<ILocation[]> {
    ...
    return this.getLocationPage(initialPage, pageSize).pipe(
      ...
      map(firstPage => {
        ...
        return { items: firstPage.items, done: false, currentPage: firstPage.metaData.page };
      }),
