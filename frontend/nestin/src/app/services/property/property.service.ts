...
export class PropertyService {
  ...
  getPropertywithImage(): Observable<Property[]> {
    return this.http.get<Property[]>('http://localhost:5050/property').pipe(
      switchMap(properties => {
        const requests = properties.map(prop => {
          return this.http
            .get<
              any[]
            >(`http://localhost:5050/propertyImage?properyID=${prop.id}`)
            .pipe(
              switchMap(propImages => {
                const imageId = propImages[0].imageId;
                return this.http
                  .get<any>(`http://localhost:5050/images/${imageId}`)
                  .pipe(
                    map(image => ({
                      ...prop,
                      imageUrl: image.name,
